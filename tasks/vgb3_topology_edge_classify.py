...
def _build_subpass_summary():
  ...
  for rec in data:
    ...
    subtask = dg.get("subtask", "classify_behaviour")
    cases = dg.get("cases")
...
promptChangeSummary = "Corner/edge ordering and curated behaviours vary per record; classify edge behaviour patterns."
highLevelSummary = (
  "Panels show squares with labeled corners. Ground truth (green) marks canonical edge behaviours or connection sets; model answers (blue) overlay predicted edges. "
  "Corner labels follow the prompt order; E0–E3 denote edges. Legends indicate behaviour classes when multiple appear."
)
...
def resultToNiceReport(result, subPass, aiEngineName: str):
  ...
  img_tags = [
    f'<img src="{fname}" alt="Topology edge classification visualization" style="max-width: 100%;">'
    for fname in file_names
  ]
  legend_html = (
    "<div style='margin-top:6px;font-size:13px;line-height:1.4;'>"
    "<b>Legend:</b> Green = ground truth edge behaviours; Blue = model answers. "
    "E0–E3 label edges; behaviours may be highlighted via arcs. Multiple cases may render—use titles for tuple order. "
    "If legend appears inside the plot, it may summarise shared behaviours.</div>")
